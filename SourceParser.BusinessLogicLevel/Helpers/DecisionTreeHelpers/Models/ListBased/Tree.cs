...
namespace SourceParser.BusinessLogicLevel.Helpers.DecisionTreeHelpers.Models.ListBased
{
    public class Tree
    {
        ...
        public Tree(int numNodes, int numClasses)
        {
            NumNodes = numNodes;
        ...
        public void BuildTree(double[][] dataX, int[] dataY)
        {
            ...
            Nodes[0].Rows = new List<int>(allRows);//асайним к нулевому узлу все исходные строки

            for (int i = 0; i < NumNodes; ++i)  // проходимся по каждому узлу
            {
                Nodes[i].NodeID = i;
                //генерим узел
                SplitInfo si = GetSplitInfo(dataX, dataY, Nodes[i].Rows, NumClasses);  // ???
                Nodes[i].SplitCol = si.SplitCol;
                Nodes[i].SplitVal = si.SplitVal;

                Nodes[i].ClassCounts = ComputeClassCts(dataY, Nodes[i].Rows, NumClasses);// ???
                Nodes[i].PredictedClass = ArgMax(Nodes[i].ClassCounts);// ???

                int leftChild = (2 * i) + 1;//установка левого и правого узлов
                int rightChild = (2 * i) + 2;

                if (leftChild < NumNodes)
                    Nodes[leftChild].Rows = new List<int>(si.LessRows);
                if (rightChild < NumNodes)
                    Nodes[rightChild].Rows = new List<int>(si.GreaterRows);
            }
        ...
        {
            Console.WriteLine("\n==========");
            Console.WriteLine("Node ID: " + Nodes[nodeID].NodeID);
            Console.WriteLine("Node split column: " + Nodes[nodeID].SplitCol);
            Console.WriteLine("Node split value: " + Nodes[nodeID].SplitVal.ToString("F2"));
            Console.Write("Node class counts: ");
            for (int c = 0; c < NumClasses; ++c)
                Console.Write(Nodes[nodeID].ClassCounts[c] + "  ");
            Console.WriteLine("");
            Console.WriteLine("Node predicted class: " + ArgMax(this.Nodes[nodeID].ClassCounts));
            Console.WriteLine("==========");
        }
        ...
        public int Predict(double[] x, bool verbose)//x - входной вектор значений предиктора, verbose - как много инфы отображать
        {
            ...
            while (true)
            {
                if (Nodes[currNodeID].Rows.Count == 0)  // at an empty node
                    break;

                if (verbose) Console.WriteLine("\ncurr node id = " + currNodeID);

                int splitCol = Nodes[currNodeID].SplitCol;
                double splitVal = Nodes[currNodeID].SplitVal;
                double v = x[splitCol];
                ...
                if (v < splitVal)
                {
                    ...
                    if (currNodeID >= Nodes.Count)
                        break;
                    result = Nodes[currNodeID].PredictedClass;
